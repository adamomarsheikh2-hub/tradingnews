...
async function loadWorld() {
  ...
  const [aircraft, marine, events] = await Promise.all([
  ...
  renderWorld(zone, aircraft, { data: { status: "loading", ships: [] } }, marine, events);

  fetchJson(`/api/ships?zone=${state.worldZone}`).then((ships) => {
    if (state.view === "world" && elements.worldZone.value === state.worldZone) {
...
async function fetchJson(path) {
  ...
  try {
    ...
    const data = text.trim().startsWith("{") ? JSON.parse(text) : { status: "error", message: text.slice(0, 180) };
...
async function loadNews() {
  ...
  try {
    ...
    render();
    setStatus(`${data.totalResults.toLocaleString()} matching stories found.`);
  } catch (error) {
...
function renderWorld(zone, aircraftResult, shipsResult, marineResult, eventsResult) {
  ...
  renderTrackingMap(zone, aircraft, ships, events);
  elements.assetTable.innerHTML = [
...
function renderTrackingMap(zone, aircraft, ships, events) {
  if (!window.maplibregl) {
    renderProjectedFallback(zone, aircraft, ships);
    return;
  }
  ...
  const paintAssets = () => {
    clearWorldMarkers();
    [...aircraft.slice(0, 120).map((item) => ["aircraft", item]), ...ships.slice(0, 120).map((item) => ["ship", item])]
      .forEach(([type, item]) => addMapMarker(type, item));
  };

  if (worldMapReady) {
    worldMap.resize();
    worldMap.easeTo({ center: [zone.lon, zone.lat], zoom: zoomForZone(zone), duration: 400 });
    paintAssets();
  } else {
    worldMap.once("load", () => {
      worldMapReady = true;
...
function ensureWorldMap(zone) {
  ...
  worldMap = new maplibregl.Map({
    ...
    zoom: zoomForZone(zone),
    attributionControl: false
  });
  worldMap.addControl(new maplibregl.NavigationControl({ showCompass: false }), "top-right");
  worldMap.addControl(new maplibregl.AttributionControl({ compact: true }), "bottom-right");
}

function projectedMarker(item, zone, type) {
  const position = projectPoint(item.lat, item.lon, zone);
  if (!position) return "";
  const label = escapeHtml(item.flight || item.name || item.id || type);
  const meta = escapeHtml(item.type || item.source || type);
  const speed = Number.isFinite(item.speed) ? `${Math.round(item.speed)} kt` : "--";
  const heading = Number(item.heading || item.course || 0);
  return `
    <button class="track-marker ${type} ${item.military ? "military" : ""}" style="left:${position.x}%;top:${position.y}%;--heading:${heading}deg" title="${label}" aria-label="${label}">
      <span class="track-symbol"></span>
      <span class="track-label"><strong>${label}</strong><small>${meta} · ${speed}</small></span>
    </button>
  `;
}

function renderProjectedFallback(zone, aircraft, ships) {
  elements.worldMap.innerHTML = '<div class="radar-map-surface"><div id="worldMarkers"></div></div>';
  elements.worldMarkers = document.querySelector("#worldMarkers");
  elements.worldMarkers.innerHTML = [
    ...aircraft.slice(0, 90).map((item) => projectedMarker(item, zone, "aircraft")),
    ...ships.slice(0, 90).map((item) => projectedMarker(item, zone, "ship"))
  ].join("");
}

function addMapMarker(type, item) {
  if (!Number.isFinite(item.lat) || !Number.isFinite(item.lon)) return;
  const element = document.createElement("button");
  element.className = `geo-marker ${type} ${item.military ? "military" : ""}`;
  element.style.setProperty("--heading", `${Number(item.heading || item.course || 0)}deg`);
  element.type = "button";
  element.innerHTML = `
    <span class="geo-symbol"></span>
    <span class="geo-tooltip">
      <strong>${escapeHtml(item.flight || item.name || item.id || type)}</strong>
      <small>${escapeHtml(item.type || type)} · ${Number.isFinite(item.speed) ? `${Math.round(item.speed)} kt` : "--"}</small>
    </span>
  `;
...
function clearWorldMarkers() {
  ...
  worldMapMarkers = [];
  if (elements.worldMarkers) elements.worldMarkers.innerHTML = "";
}
...
function zoomForZone(zone) {
  if (zone.span <= 1.5) return 9.6;
  if (zone.span <= 4) return 7.4;
  if (zone.span <= 6) return 6.7;
  return 5.7;
}
