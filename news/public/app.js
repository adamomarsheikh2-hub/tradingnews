...
function renderTrackingMap(zone, aircraft, ships, events) {
  if (!window.L) {
    elements.worldMap.innerHTML = '<div class="map-fallback">Map library failed to load.</div>';
    return;
  }

  if (!worldMap) {
    worldMap = L.map(elements.worldMap, {
      zoomControl: true,
  ...
  worldMap.setView([zone.lat, zone.lon], zoomForZone(zone));
  window.setTimeout(() => worldMap.invalidateSize(), 80);

  L.circle([zone.lat, zone.lon], {
    radius: zone.span * 62000,
    color: "#7fa5ff",
    weight: 1,
    opacity: 0.46,
    fillColor: "#7fa5ff",
  ...
  aircraft.slice(0, 90).forEach((item) => {
    if (!Number.isFinite(item.lat) || !Number.isFinite(item.lon)) return;
    L.marker([item.lat, item.lon], {
      icon: aircraftIcon(item.heading, item.military)
    })
      .bindPopup(assetPopup(item, "Aircraft"))
      .addTo(worldLayer);
  });

  ships.slice(0, 90).forEach((item) => {
    if (!Number.isFinite(item.lat) || !Number.isFinite(item.lon)) return;
    L.marker([item.lat, item.lon], {
      icon: shipIcon(item.course)
    })
      .bindPopup(assetPopup(item, "Ship"))
      .addTo(worldLayer);
  });

  events.slice(0, 5).forEach((event, index) => {
    const offset = (index - 2) * 0.08 * zone.span;
    L.marker([zone.lat + offset, zone.lon - offset], {
      icon: eventIcon()
    })
...
function aircraftIcon(heading = 0, military = false) {
  return L.divIcon({
    className: "tracker-icon-wrap",
    html: `<span class="tracker-icon aircraft-icon ${military ? "military" : ""}" style="--heading:${Number(heading) || 0}deg"></span>`,
    iconSize: [28, 28],
    iconAnchor: [14, 14],
    popupAnchor: [0, -12]
  });
...
function shipIcon(course = 0) {
  return L.divIcon({
    className: "tracker-icon-wrap",
    html: `<span class="tracker-icon ship-icon" style="--heading:${Number(course) || 0}deg"></span>`,
    iconSize: [28, 28],
    iconAnchor: [14, 14],
    popupAnchor: [0, -12]
  });
...
function eventIcon() {
  return L.divIcon({
    className: "tracker-icon-wrap",
    html: '<span class="tracker-icon event-icon"></span>',
    iconSize: [24, 24],
    iconAnchor: [12, 12],
    popupAnchor: [0, -12]
  });
...
function assetPopup(item, kind) {
  ...
  const heading = Number.isFinite(item.heading || item.course) ? `${Math.round(item.heading || item.course)} deg` : "--";
...
function eventPopup(event) {
  return `
    <div class="asset-popup">
      <strong>${escapeHtml(event.source || "World Event")}</strong>
...
function zoomForZone(zone) {
  if (zone.span <= 1.5) return 10;
  if (zone.span <= 4) return 8;
  if (zone.span <= 6) return 7;
  return 6;
}
